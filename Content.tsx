...
import {getSvgPathFromStroke, saveToInventory, saveMaskToInventory} from './utils';
  ...
  const handleSaveToInventory = async (box: any, index: number) => {
    ...
    try {
      ...
      if (detectType === 'Segmentation masks') {
        inventoryItem = await saveMaskToInventory(box, imageSrc);
      } else {
        inventoryItem = await saveToInventory(box, imageSrc);
      }
      ...
      notification.textContent = `${box.label} saved to inventory`;
      ...
      notification.textContent = `Error saving to inventory: ${errorMessage}`;
        ...
        {detectType === '2D bounding boxes' &&
          boundingBoxes2D.map((box, i) => (
            <div
              ...
              <div className="bg-[#3B68FF] text-white absolute left-0 top-0 text-sm px-1 flex items-center">
                <span className="mr-2">{box.label}</span>
                <button 
                  onClick={() => handleSaveToInventory(box, i)}
                  className="ml-auto bg-white bg-opacity-30 rounded p-1 hover:bg-opacity-50 flex items-center"
                  title="Save to inventory"
                  disabled={savingBoxIndex === i}
                >
                  {savingBoxIndex === i ? (
          ...
          boundingBoxMasks.map((box, i) => (
            <div
              ...
              <div className="w-full top-0 h-0 absolute">
                <div className="bg-[#3B68FF] text-white absolute -left-[2px] bottom-0 text-sm px-1 flex items-center">
                  <span className="mr-2">{box.label}</span>
                  <button 
                    onClick={() => handleSaveToInventory(box, i)}
                    className="ml-auto bg-white bg-opacity-30 rounded p-1 hover:bg-opacity-50 flex items-center"
