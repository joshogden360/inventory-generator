...
export function LeftPanelInventory() {
  ...
  return (
    <div className={`bg-base-100 text-base-content border-r border-base-300 flex-shrink-0 transition-all duration-300 flex flex-col shadow-lg ${
        ...
        <div className="p-2 h-full flex flex-col items-center justify-start gap-3 pt-3">
          ...
          {inventoryItems.length > 0 && (
            <div className="tooltip tooltip-right" data-tip={`${inventoryItems.length} items`}>
              <div className="badge badge-primary badge-md font-semibold">{inventoryItems.length}</div>
            </div>
          )}
        </div>
      )}
      ...
      {leftPanelExpanded && (
        <div className="flex flex-col h-full overflow-hidden">
          ...
          {/* Items List */}
          <div className="flex-1 overflow-y-auto p-3 space-y-2 scrollbar-thin scrollbar-thumb-base-300 scrollbar-track-base-100">
            {filteredItems.length === 0 ? (
              <div className="text-center opacity-70 text-sm py-8 px-2">
                {inventoryItems.length === 0 
...
function LeftPanelInventoryItem({ item, onDelete }: LeftPanelInventoryItemProps) {
  return (
    <div className="card card-compact bg-base-200/50 hover:bg-base-200 shadow group transition-all">
      <div className="card-body p-2">
        <div className="flex items-start gap-2">
          <figure className="w-12 h-12 bg-base-300 rounded-md overflow-hidden flex-shrink-0">
            <img 
              src={item.imageUrl} 
              alt={item.label}
          ...
          <div className="flex-1 min-w-0">
            <h4 className="font-semibold text-sm truncate group-hover:text-primary transition-colors">{item.label}</h4>
            {item.category && (
        ...
        {item.tags && item.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-1.5">
            {item.tags.slice(0, 3).map((tag, i) => (
              <div key={i} className="badge badge-neutral badge-xs">
                {tag}
              </div>
            ))}
            {item.tags.length > 3 && (
              <div className="badge badge-ghost badge-xs">+{item.tags.length - 3}</div>
            )}
          </div>
        )}
      </div>
